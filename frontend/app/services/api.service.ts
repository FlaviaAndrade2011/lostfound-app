...
export class ApiService {
    ...
    getMyStudentItems(): Observable<Item[]> {
        const token = localStorage.getItem('student_token');
        const headers = token ? new HttpHeaders({ Authorization: `Bearer ${token}` }) : undefined;
        return this.http.get<Item[]>(`${API_URL}/items/student/mine`, { headers });
    }

    updateMyStudentItem(itemId: number, data: FormData): Observable<any> {
        const token = localStorage.getItem('student_token');
        const headers = token ? new HttpHeaders({ Authorization: `Bearer ${token}` }) : undefined;
        return this.http.put(`${API_URL}/items/student/${itemId}`, data, { headers });
    }

    deleteMyStudentItem(itemId: number): Observable<any> {
        const token = localStorage.getItem('student_token');
        const headers = token ? new HttpHeaders({ Authorization: `Bearer ${token}` }) : undefined;
        return this.http.delete(`${API_URL}/items/student/${itemId}`, { headers });
    }
